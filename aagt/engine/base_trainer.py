...
from aagt.utils.summary_board import SummaryBoard
from aagt.utils.timer import Timer
from aagt.utils.torch import all_reduce_tensors, release_cuda, initialize
from aagt.engine.logger import Logger
import wandb
import copy
...
class BaseTrainer(abc.ABC):
    ...
    def save_snapshot(self, filename):
        ...
        if self.use_ema and self.ema_model is not None:
            state_dict['state_dict_ema'] = self.ema_model.module.state_dict()
    ...
    def load_snapshot(self, snapshot, fix_prefix=True):
        ...
        # --- New: Load EMA ---
        if self.use_ema and 'state_dict_ema' in state_dict and self.ema_model is not None:
            self.logger.info('Loading EMA parameters...')
            self.ema_model.module.load_state_dict(state_dict['state_dict_ema'])
    ...
    def register_model(self, model):
        ...
        if self.use_ema:
            self.logger.info(f'Initializing EMA with decay: {self.ema_decay}')
            self.ema_model = ModelEMA(model, decay=self.ema_decay)
    ...
    def optimizer_step(self, iteration):
        if iteration % self.grad_acc_steps == 0:
            ...
            # Update EMA
            if self.use_ema and self.ema_model is not None:
                actual_model = self.model.module if self.distributed else self.model
                self.ema_model.update(actual_model)
