...
from aagt.modules.ops import grid_subsample, radius_search
from aagt.utils.torch import build_dataloader

def extract_top_ab_torch(cur_nor, points, lengths, a, b):

    half_n = int(lengths[0] // 2)
    half_nb = int(lengths[1] // 2)
    assert a <= half_n, f"a={a} beyond {half_n}"
    assert b <= half_nb, f"b={b} beyond {half_nb}"
    
    front_curnor = cur_nor[:half_n, :]
    back_curnor = cur_nor[half_n:, :]
    front_points = points[:half_n, :]
    back_points = points[half_n:, :]

    front_first_col = front_curnor[:, 0] 
    _, top_a_indices = torch.topk(front_first_col, k=a, dim=0)
    ref_feats_top = front_curnor[top_a_indices, :] 
    
    back_first_col = back_curnor[:, 0]   
    _, top_b_indices = torch.topk(back_first_col, k=b, dim=0)
    src_feats_top = back_curnor[top_b_indices, :] 

    ref_points_extra = front_points[top_a_indices, :]
    src_points_extra = back_points[top_b_indices, :]
    
    return ref_feats_top, src_feats_top, ref_points_extra, src_points_extra

# Stack mode utilities

def precompute_subsample(points, lengths, feats_extra, num_stages, voxel_size, num_neighbors, subsample_ratio):
    ...
    ref_length = lengths_list[-1][0].item()
    src_length = len(points_list[-1].detach()) - lengths_list[-1][0].item()
    # ref_feats_top, src_feats_top, ref_points_extra, src_points_extra = extract_top_ab_torch(feats_extra, points_ori, lengths_ori, ref_length, src_length)
    feats_extra_list[-1], feats_extra_list[0], points_extra_list[-1], points_extra_list[0] = extract_top_ab_torch(feats_extra, points_ori, lengths_ori, ref_length, src_length)

    return {
        'points': points_list,
        'lengths': lengths_list,
        'points_extra': points_extra_list,
        'feats_extra': feats_extra_list,
    }
...
def registration_collate_fn_stack_mode(
    ...
    feats_extra = torch.cat(collated_dict.pop('ref_curnor') + collated_dict.pop('src_curnor'), dim=0) # 获取额外特征
    ...
    if precompute_data:
        input_dict = precompute_subsample(points, lengths, feats_extra, num_stages, voxel_size, num_neighbors, subsample_ratio)
