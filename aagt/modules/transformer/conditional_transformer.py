...
from aagt.modules.transformer.vanilla_transformer import TransformerLayer
...
class RPEConditionalTransformer(nn.Module):
    ...
    def forward(self, feats0, feats1, embeddings0, embeddings1, feats0_extra, feats1_extra, embeddings0_extra, embeddings1_extra, masks0=None, masks1=None):
        attention_scores = []
        for i, block in enumerate(self.blocks):
            if block == 'self':
                feats0, scores0 = self.layers[i](feats0, feats0, embeddings0, feats0_extra, embeddings0_extra, memory_masks=masks0)
                feats1, scores1 = self.layers[i](feats1, feats1, embeddings1, feats1_extra, embeddings1_extra, memory_masks=masks1)
            else:
                if self.parallel:
