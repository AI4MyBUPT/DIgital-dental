...
from aagt.modules.layers import build_dropout_layer
from aagt.modules.transformer.output_layer import AttentionOutput
...
class RPEMultiHeadAttention(nn.Module):
    ...
    def forward(self, input_q, input_k, input_v, embed_qk, c_weights, embed_c, key_weights=None, key_masks=None, attention_factors=None):
        r"""Scaled Dot-Product Attention with Pre-computed Relative Positional Embedding (forward)
        ...
        v = rearrange(self.proj_v(input_v), 'b m (h c) -> b h m c', h=self.num_heads)
        p = rearrange(self.proj_p(embed_qk), 'b n m (h c) -> b h n m c', h=self.num_heads)
        c = rearrange(self.proj_p(embed_c), 'b n m (h c) -> b h n m c', h=self.num_heads)
        ...
        # attention_scores_c = torch.einsum('bhnc,bhnmc->bhnm', q1, c)
        attention_scores_c = torch.einsum('bhnc,bhnmc->bhnm', q, c)
        attention_scores_c = attention_scores_c * c_weights.unsqueeze(1).unsqueeze(1)
        attention_scores = (attention_scores_e + attention_scores_p + attention_scores_c) / self.d_model_per_head ** 0.5
        if attention_factors is not None:
            attention_scores = attention_factors.unsqueeze(1) * attention_scores
        if key_weights is not None:
            attention_scores = attention_scores * key_weights.unsqueeze(1).unsqueeze(1)
