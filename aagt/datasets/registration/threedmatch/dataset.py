...
from aagt.utils.pointcloud import (
    random_sample_rotation,
    random_sample_rotation_v2,
    get_transform_from_rotation_translation,
    uniform_2_sphere
)
from aagt.utils.registration import get_correspondences, compute_overlap, compute_overlap_mask
...
class ThreeDMatchPairDataset(torch.utils.data.Dataset):
    ...
    def _load_point_cloud(self, file_name, cur_nor):
        ...
        if self.point_limit is not None and points.shape[0] > self.point_limit:
            ...
            cur_nor_i = cur_nor[indices]
        return points, cur_nor_i
    ...
    def _random_crop(self, ref_points, src_points, ref_masks, src_masks, p_keep):
        ...
        centroid = np.mean(ref_points.numpy(), axis=0)
        # centroid = ref_points.mean(dim=0)
        ...
        rand_xyz = uniform_2_sphere()
        centroid = np.mean(src_points.numpy(), axis=0)
        # centroid = src_points.mean(dim=0)
    ...
    def __getitem__(self, index):
        ...
        data_dict['ref_curnor'] = metadata['cur_nor0']
        data_dict['src_curnor'] = metadata['cur_nor1']
        # get transformation
        rotation = metadata['rotation']
        translation = metadata['translation']

        ref_points, data_dict['ref_curnor'] = self._load_point_cloud(metadata['pcd0'], data_dict['ref_curnor'])
        src_points, data_dict['src_curnor'] = self._load_point_cloud(metadata['pcd1'], data_dict['src_curnor'])
