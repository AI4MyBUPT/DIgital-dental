...
from aagt.modules.ops import point_to_node_partition, index_select
from aagt.modules.registration import get_node_correspondences

from aagt.modules.dual_matching import PointDualMatching

from aagt.modules.geotransformer import (
    GeometricTransformer,
    SuperPointMatching,
    SuperPointTargetGenerator,
)

from aagt.modules.registration import HypothesisProposer
...
class PARE_Net(nn.Module):
    def __init__(self, cfg):
        ...
        self.net = cfg.backbone.net
        self.encoder = create_encoder(self.net)
        self.mlp = create_mlp('default')
    ...
    def forward(self, data_dict):
        ...
        # extra data
        ref_points_extra = data_dict['points_extra'][-1].detach().float()
        src_points_extra = data_dict['points_extra'][0].detach().float()
        ref_points_extra_feats = data_dict['feats_extra'][-1].detach().float()
        src_points_extra_feats = data_dict['feats_extra'][0].detach().float()

        src_feats_extra_one, ref_feats_extra_one = self.encoder(data_dict)
        ref_feats_extra_one = ref_feats_extra_one.T
        src_feats_extra_one = src_feats_extra_one.T

        ref_curvature = ref_points_extra_feats[:, 0:1] # [N, 1]
        ref_normals = ref_points_extra_feats[:, 1:4]   
        src_curvature = src_points_extra_feats[:, 0:1]
        src_normals = src_points_extra_feats[:, 1:4]
        ...
        ref_feats_c, src_feats_c, scores_list = self.transformer(
            ...
            ref_points_extra.unsqueeze(0),
            src_points_extra.unsqueeze(0),
            ref_feats_extra_one,
            src_feats_extra_one,
            ref_curvature.unsqueeze(0),
            src_curvature.unsqueeze(0),
            ref_normals.unsqueeze(0),
            src_normals.unsqueeze(0),
        )
        ...
        output_dict['ref_feats_extra'] = ref_feats_extra_one
        output_dict['src_feats_extra'] = src_feats_extra_one
