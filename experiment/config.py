...
from aagt.utils.common import ensure_dir
...
# data
_C.data = edict()
_C.data.dataset_root = '/../../AAGT/data_path/3DS'
_C.data.metadata_root = osp.join(_C.root_dir, 'data', '3DS', 'metadata')
...
_C.eval.acceptance_radius = 1  # 0.1
_C.eval.inlier_ratio_threshold = 0.05
_C.eval.rmse_threshold = 1 # 0.2
_C.eval.rre_threshold = 5.0 # 15.0
_C.eval.rte_threshold = 3.0 # 0.3
_C.eval.feat_rre_threshold = 5.0
# ransac
_C.ransac = edict()
_C.ransac.distance_threshold = 0.3 #0.05
_C.ransac.num_points = 4
_C.ransac.num_iterations = 10000

# optim
_C.optim = edict()
_C.optim.lr = 1e-4
_C.optim.lr_decay = 0.98
_C.optim.lr_decay_steps = 4
_C.optim.weight_decay = 1e-6
_C.optim.max_epoch = 250
_C.optim.grad_acc_steps = 1
_C.optim.ema_decay = 0.995

# model - backbone
_C.backbone = edict()
_C.backbone.num_stages = 4
_C.backbone.num_neighbors = [35] * _C.backbone.num_stages  # we use constant neighbors
_C.backbone.init_voxel_size = 0.75
...
# model - Global
_C.model = edict()
_C.model.ground_truth_matching_radius = 1.0
...
_C.geotransformer.sigma_d = 4.8 # 0.1
...
_C.coarse_loss.log_scale = 50
_C.coarse_loss.positive_overlap = 0.1

# loss - Fine level
_C.fine_loss = edict()
_C.fine_loss.positive_radius = 1.0  # 0.05
_C.fine_loss.negative_radius = 5.0
_C.fine_loss.positive_margin = 2.0  # 0.1
_C.fine_loss.negative_margin = 8.0
